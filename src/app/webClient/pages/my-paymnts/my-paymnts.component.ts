...
export class MyPaymntsComponent implements OnInit{
  ...
  constructor(private http:HttpClient,private paymentService:PaymentService,private planService:PlanService,private authService:AuthService){}
  ...
  getAuthUserPayments(id:number){
    this.http.get('http://localhost:8080/api/web/payments/'+id).subscribe((res:any) => {
  ...
  getSubscriptions(id:number){
    return this.http.get<SubscriptionWithPayments[]>('http://localhost:8080/api/web/subscription-members/'+id).pipe(
      map((subscriptions: SubscriptionWithPayments[]) =>
