...
@Component({
  ...
  providers: [MessageService,ConfirmationService,{ provide: NgControl, useExisting: forwardRef(() => SessionComponent) }]
})
...
getAllSessionMembers() {
  ...
  onScanSuccess(resultString: string) {
    ...
    this.sessionService.canEnter(resultString).subscribe((res:any) => {
      ...
      this.messageService.add({severity:'success', summary: 'Successful', detail: 'entering successfuly', life: 3000});
      }else if(res === 2){
        this.getAllSessionMembers();
        this.sessionService.playGoodbyeAudio();
        this.messageService.add({severity:'warn', summary: 'Successful', detail: 'leaving successfuly', life: 3000});
      }
      else{
        this.messageService.add({severity:'error', summary: 'not Allowed', detail: 'not allowed to enter'+res, life: 3000});
