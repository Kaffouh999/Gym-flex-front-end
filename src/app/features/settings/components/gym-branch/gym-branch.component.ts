...
export class GymBranchComponent implements OnInit {
    ...
    constructor(
        ...
        private gymBrancheservice: GymBranchService,
    ...
    deleteSelectedgymBranches() {
        this.confirmationService.confirm({
            message:
                "Are you sure you want to delete the selected gymBranches?",
            header: "Confirm",
            icon: "pi pi-exclamation-triangle",
            accept: () => {
                this.gymBranches = this.gymBranches.filter(
                    (val) => !this.selectedgymBranches.includes(val)
                );
                this.selectedgymBranches = null;
                this.messageService.add({
                    severity: "success",
                    summary: "Successful",
                    detail: "gymBranches Deleted",
                    life: 3000,
                });
            },
        });
    ...
    deletegymBrancheInput(gymBrancheInput: GymBranch) {
        this.confirmationService.confirm({
            message:
                "Are you sure you want to delete " + gymBrancheInput.name + "?",
            header: "Confirm",
            icon: "pi pi-exclamation-triangle",
            accept: () => {
                this.gymBrancheservice
                    .deleteBranch(gymBrancheInput.id)
                    .subscribe({
                        next: (res: any) => {
                            this.getAllGymBranches();
                            this.messageService.add({
                                severity: "success",
                                summary: "Successful",
                                detail: "gymBrancheInput Deleted",
                                life: 3000,
                            });
                        },
                        error: (error: any) => {
    ...
    savegymBrancheInput() {
        ...
        if (this.gymBrancheInput.id === undefined) {
            this.gymBrancheservice.addBranch(this.gymBrancheInput).subscribe({
                next: (res: any) => {
                    this.getAllGymBranches();
                    this.messageService.add({
                        severity: "success",
                        summary: "Successful",
                        detail: "gymBranche added successfuly",
                        life: 3000,
                    });
                },
                error: (error: any) => {
            ...
            this.gymBrancheservice
                .updateBranch(this.gymBrancheInput.id, this.gymBrancheInput)
                .subscribe({
                    next: (res: any) => {
                        this.getAllGymBranches();
                        this.messageService.add({
                            severity: "success",
                            summary: "Successful",
                            detail: "gymBranche updated successfuly",
                            life: 3000,
                        });
                    },
                    error: (error: any) => {
    ...
    getAllGymBranches() {
        ...
        this.gymBrancheservice.getALlBranches().subscribe({
            next: (res: any) => {
    ...
    remplirGymBranch(): void {
        this.gymBrancheInput = {
            ...
            sessionDurationAllowed: this.gymbranchFrom.get(
                "sessionDurationAllowed"
            ).value,
        };
    }
