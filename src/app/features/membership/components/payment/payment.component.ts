...
@Component({
    ...
    providers: [
        MessageService,
        ConfirmationService,
        { provide: NgControl, useExisting: forwardRef(() => PaymentComponent) },
    ],
})
export class PaymentComponent {
    ...
    fillPayment(): void {
        let shouldPay: number =
            this.PaymentForm.get("subscriptionMember").value.plan.price -
            (this.PaymentForm.get("subscriptionMember").value.plan.price *
                this.PaymentForm.get("subscriptionMember").value
                    .discountPercentage) /
                100;
        this.paymentInput = {
    ...
    onScanSuccess(resultString: string) {
        ...
        this.subcriptionService
            .getSubscriptionByQrCode(resultString)
            .subscribe((res: any) => {
    ...
    exportExcel() {
        let selectedPayments = this.selectedPyaments.map((col: Payment) => ({
            member:
                col.subscriptionMember.member.onlineUser.firstName +
                " " +
                col.subscriptionMember.member.onlineUser.lastName,
            plan: col.subscriptionMember.plan.name,
            shouldPay:
                col.subscriptionMember.plan.price -
                (col.subscriptionMember.plan.price *
                    col.subscriptionMember.discountPercentage) /
                    100,
            amountPayed: col.amountPayed,
            paymentDate: this.datePipe.transform(col.paymentDate, "dd/MM/yyyy"),
            payedMember:
                col.payedMember.onlineUser.firstName +
                " " +
                col.payedMember.onlineUser.lastName,
        }));
