...
@Component({
  ...
  providers: [MessageService,ConfirmationService,{ provide: NgControl, useExisting: forwardRef(() => AssuranceComponent) }]
...
saveAssurance() {
  ...
  if (this.assuranceInput.id === undefined) {
      this.assuranceService.addAssurance(this.assuranceInput).subscribe((res: any) => {
          this.getAllAssurances();
          this.hideDialog();
...
exportExcel(){
    let selectedAssurances = this.selectedAssurances.map((col:AssuranceMember) => ( {
    ...
    this.exporterService.exportExcel(selectedAssurances,this._selectedColumns,"Assurances");
...
exportPdf(){
    ...
    this.exporterService.exportPdf(selectedAssurances,this._selectedColumns,"Assurances");
}
clear(table: Table) {
    table.clear();
}
