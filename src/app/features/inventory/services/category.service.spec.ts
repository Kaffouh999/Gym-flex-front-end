...
describe('CategoryService', () => {
  ...
  it('should add a category', () => {
    const category: Category = { id: 1, name: 'Category 1', description: 'Description 1', isForClient: false, isForInventory: false };
  ...
  it('should delete a category', () => {
    ...
    const req = httpMock.expectOne(`http://localhost:8080/api/categories/${categoryId}`);
    expect(req.request.method).toBe('DELETE');
    req.flush({ success: true });
  });

  it('should update a category', () => {
    const updatedCategory: Category = { id: 1, name: 'Updated Category', description: 'Description 1', isForClient: false, isForInventory: false };
    const categoryId = 1;
    service.updateCategory(categoryId, updatedCategory).subscribe(response => {
